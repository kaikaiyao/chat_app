...
const ChatWindow = ({
  ...
  // New state to track if the user is at the bottom of the chat
  const [atBottom, setAtBottom] = useState(true);
  const virtuosoRef = useRef(null);

  // Refs for debouncing "think" events
  const thinkBufferRef = useRef("");
  const lastThinkUpdateRef = useRef(Date.now());
  ...
  // Auto-scroll when streaming if the user is at the bottom
  useEffect(() => {
    if (isProcessing && atBottom && virtuosoRef.current) {
  ...
  }, [messages, isProcessing, atBottom]);
    ...
    const processEvent = (data) => {
      switch (data.type) {
        ...
        case "think_start":
          ...
          break;
        case "think":
          // Accumulate chunks and update debounced
          thinkBufferRef.current += data.content;
          if (Date.now() - lastThinkUpdateRef.current > 50) {
            setMessages((prev) =>
              prev.map((msg) =>
                msg.id === thinkingMessageId
                  ? { ...msg, content: msg.content + thinkBufferRef.current }
                  : msg
              )
            );
            lastThinkUpdateRef.current = Date.now();
